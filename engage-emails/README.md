...
### Configure a New Integration in Adobe Commerce in case if you do no use the IMS authentication
...
4. Note down the integration credentials into your local .env file:
   COMMERCE_BASE_URL=example_base_url
   COMMERCE_CONSUMER_KEY=example_consumer_key
   COMMERCE_CONSUMER_SECRET=example_consumer_secret
   COMMERCE_ACCESS_TOKEN=example_access_token
   COMMERCE_ACCESS_TOKEN_SECRET=example_access_token_secret
...
bin/magento events:subscribe observer.sales_order_save_commit_after --fields=*
```
...
1. Access the Configuration Page:
   Once the application is deployed, you will be able to access the configuration UI. The specific URL will be provided upon successful deployment of the Adobe I/O App Builder application. It typically follows a pattern like `https://[your-app-builder-url]/index.html#/configuration`.
2. Go to the ***Configuration*** page and fill the following fields:
...
4. Go to the **Automations** page and create a new automation.
...
5. Click 'Save Automation' button.

**Note on `env.dist`:** The configuration for this microservice is managed entirely through the exposed user interface within the Adobe I/O App Builder application, as detailed in the steps above. Therefore, an `env.dist` file for environment variables is not required for this configuration.
